...
/**
 * <i>Based on LinkedList.java in OpenJDK 8.</i>
 *
 * <p>Doubly-linked circular list implementation of the {@code List} and {@code Deque} interfaces.
 * Implements all optional list operations, and permits all elements (including {@code null}).
 *
 * <p>All of the operations perform as could be expected for a doubly-linked list. Operations that
 ...
 *
 * <p>The iterators returned by this class's {@code iterator} and {@code listIterator} methods are
 * <i>fail-fast</i>: if the list is structurally modified at any time after the iterator is created,
 * in any way except through the Iterator's own {@code remove} or {@code add} methods, the iterator
 * will throw a {@link ConcurrentModificationException}. Thus, in the face of concurrent
 ...
 */
public class CircularLinkedList<E> extends AbstractSequentialList<E>
  ...
  /** Inserts element {@code e} before non-null {@link Node} {@code succ}. */
  ...
  static final class LLSpliterator<E> implements Spliterator<E> {
    ...
    final CircularLinkedList<E> list; // null OK unless traversed
    Node<E> current; // current node; null until initialized
    int est; // size estimate; -1 until first needed
    int expectedModCount; // initialized when est set
    int batch; // batch size for splits

    LLSpliterator(CircularLinkedList<E> circularLinkedList, int est, int expectedModCount) {
      this.list = circularLinkedList;
      this.est = est;
    ...
    final int getEst() {
      ...
      final CircularLinkedList<E> lst;
      if ((s = est) < 0) {
        if ((lst = list) == null) {
          s = est = 0;
        } else {
